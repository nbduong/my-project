...
const API_KEY = 'AIzaSyAMm-idYCqwin9Bwbzq24PCvKm-lrZDtMc'; // API key của Gemini
...
export default function Layout() {
  ...
  const sendMessage = async (): Promise<void> => {
    ...
    const productList = products
      .map(
        (product) =>
          `Name: ${product.name}, Code: ${product.productCode}, FinalPrice: ${product.finalPrice ?? 'N/A'}, DiscountPercent: ${product.discountPercent ?? 'N/A'}, Brand: ${product.brandName ?? 'N/A'}, Category: ${product.categoryName ?? 'N/A'}, Description: ${product.description ?? 'N/A'}`
      )
      .join('\n');
    ...
    const history = newMessages
      ...
      .join('\n');
    const prompt = `Bạn là một trợ lý AI của cửa hàng GearVN, chuyên gia về cấu hình và linh kiện máy tính. Dựa trên lịch sử hội thoại và danh sách sản phẩm dưới đây, trả lời câu hỏi hiện tại hoặc gợi ý sản phẩm phù hợp một cách ngắn gọn trong khoảng 3 dòng đến 5 dòng là nhiều nhất, trực tiếp và liên quan đến ngữ cảnh. Nếu người dùng yêu cầu gợi ý sản phẩm, hãy sử dụng danh sách sản phẩm để đưa ra gợi ý cụ thể, bao gồm tên sản phẩm, tuyệt đối không đưa ra id sản phẩm, giá và lý do gợi ý, nếu không có sản phẩm cần thiết trong danh sách sản phẩm thì hãy lấy trên mạng. Danh sách sản phẩm:\n${productList}\n\nLịch sử hội thoại:\n${history}\n\nUser: ${chatInput}`;
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { text: `Error: Could not connect to Gemini server - ${error}`, sender: 'bot' },
      ]);
    }
  };
  ...
  const addToCart = useCallback((product: Product, quantity: number = 1) => {
    setCartItems((prevItems) => {
      ...
      return [...prevItems, { product: { ...product, salePrice: product.salePrice ?? 0 }, quantity }];
    });
  }, []);
    ...
    if (pathSegments[0] === 'products') {
      ...
      if (categoryName) {
        breadcrumbItems.push({
          label: categoryName,
          path: `/products?categoryName=${encodeURIComponent(categoryName)}`,
        });
      } else if (query) {
        breadcrumbItems.push({
          label: `Tìm kiếm: ${query}`,
          path: `/products?query=${encodeURIComponent(query)}`,
        });
      }
    ...
    <div className='min-h-screen bg-gray-100 flex flex-col'>
        ...
        <div className='container mx-auto px-4 py-4 flex items-center justify-between'>
          ...
          <div className='flex-1 mx-4 relative' ref={searchRef}>
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className='absolute top-full left-0 w-[500px] bg-white border rounded-lg mt-1 shadow-lg max-h-60 overflow-y-auto z-50'>
                  ...
                  >
                    ...
                    <div className='flex flex-col'>
                      ...
                      <span className='text-sm text-gray-500'>
                        {(product.finalPrice ?? product.salePrice ?? 0).toLocaleString()} VNĐ
                      </span>
                    </div>
                  </div>
                ))}
            ...
            <ul className='space-y-4'>
              ...
              {categories.map((category) => (
                <li key={category.name}>
                  <Link
                    to={`/products?categoryName=${encodeURIComponent(category.name)}`}
                    className={`flex items-center text-gray-800 hover:text-[#3B82F6]uctor
                    ${isCategoryActive(category.name) ? 'text-[#3B82F6]' : ''
                      }`}
                    onClick={() => setShowMobileMenu(false)}
                  >
                    {getCategoryIcon(category.name)}
      ...
      <div className='fixed bottom-4 right-4 z-50'>
        <button
          ...
          className='bg-[#3B82F6] text-white p-3 rounded-full shadow-lg hover:bg-[#2563EB] transition-all duration-200'
        ...
        {showChatbot && (
          <div
            ...
            className='absolute bottom-16 right-0 w-80 bg-white border border-gray-300 rounded-lg shadow-lg p-4 max-h-96 flex flex-col'
          >
            <div className='flex justify-between items-center mb-2'>
              <h3 className='text-lg font-semibold text-[#1E3A8A]'>Trợ lý ảo GearBot</h3>
              <button
            ...
            <div className='flex-1 overflow-y-auto mb-2'>
              {messages.map((msg, index) => (
                <div
                  key={index}
                  className={`p-2 rounded-md mb-2 text-sm ${msg.sender === 'user' ? 'bg-[#3B82F6] text-white ml-8' : 'bg-gray-200 text-gray-800 mr-8'
                    }`}
                >
                  {msg.text}
                </div>
            ...
            <div className='flex gap-2'>
              <input
                ...
                className='flex-1 p-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#3B82F6]'
              />
              <button
                onClick={sendMessage}
                className='bg-[#3B82F6] text-white px-3 py-2 rounded-md text-sm hover:bg-[#2563EB]'
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
            ...
            <div>
              ...
              <ul className='text-sm space-y-2'>
                <li>
                  <Link to='/' className='hover:text-[#FBBF24] transition-colors'>
                    Trang chủ
                  </Link>
                </li>
                <li>
                  <Link to='/products' className='hover:text-[#FBBF24] transition-colors'>
                    Sản phẩm
                  </Link>
                </li>
                <li>
                ...
                <li>
                  <Link to='/about' className='hover:text-[#FBBF24] transition-colors'>
                    Về chúng tôi
                  </Link>
                </li>
                <li>
                  <Link to='/contact' className='hover:text-[#FBBF24] transition-colors'>
                    Liên hệ
                  </Link>
                </li>
              </ul>
            ...
            <div>
              ...
              <div className='flex space-x-4'>
                <a
                  href='https://web.facebook.com/Zawser19'
                  target='_blank'
      ...
      {showBackToTop && (
        <button
          ...
          className='fixed bottom-4 right-20 p-3 bg-[#3B82F6] text-white rounded-full shadow-lg hover:bg-[#2563EB] transition-all duration-200'
